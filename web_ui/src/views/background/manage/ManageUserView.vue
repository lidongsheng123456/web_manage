  ...
  <div>
    <el-form ref="queryFormRef" :inline="true" :model="queryParams" label-width="70px">
      <el-form-item label="用户名" prop="username">
        <el-input v-model="queryParams.username" placeholder="请输用户名" style="width: 200px" />
      </el-form-item>
      <el-form-item label="手机号" prop="phone">
        <el-input v-model="queryParams.phone" placeholder="请输入手机号" style="width: 200px" />
      </el-form-item>
      <el-form-item label="邮箱" prop="email">
        <el-input v-model="queryParams.email" placeholder="请输入邮箱" style="width: 200px" />
      </el-form-item>
      <el-form-item>
    ...
    <el-table v-loading="loading" :data="tableData" :default-sort="{ prop: 'id', order: 'descending' }"
      ...
      <el-table-column align="center" label="角色" prop="roles">
        <template #default="scope">
          <el-select v-model="scope.row.roles[0].role_code" placeholder="查看角色" size="small" style="width: 70px">
            <el-option v-for="item in scope.row.roles" :key="item.role_code" :label="item.role_code"
      ...
      <el-table-column align="center" label="权限" prop="permissions">
        <template #default="scope">
          <el-select v-model="scope.row.permissions[0].permission_code" placeholder="查看权限" size="small"
            style="width: 70px">
            <el-option v-for="item in scope.row.permissions" :key="item.permission_code" :label="item.permission_code"
      ...
      <el-form ref="formRef" :inline="true" :model="form" :rules="rules" align="center" label-width="80px">
        <el-form-item label="头像" style="display: flex;align-items: center;justify-content: space-around">
          <el-upload :action="uploadUrl + '/common/files/upload'" :before-upload="beforeAvatarUpload"
            :on-success="handleAvatarSuccess" :show-file-list="false" class="avatar-uploader" style="margin: 0 auto">
  ...
  formRef.value?.validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateUser(form.value as AdminUser).then(res => {
...
// 控制删除
const handleDelete = (row: Partial<AdminUser>) => {
...
// 控制更新
const handleUpdate = (row: Partial<AdminUser>) => {
  resetFrom();
...
<style lang="scss">
@use "@/assets/css/admin/avatar-uploader";
</style>
