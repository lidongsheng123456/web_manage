...
// 引用子组件实例
const assignRoleViewRef = ref<InstanceType<typeof AssignRoleView>>();
// 上传的ip和端口号
const uploadUrl = import.meta.env.VUE_APP_BASEURL
// 表单
const form = ref<Partial<Role>>({
  roleCode: null as unknown as string,
  ...
  formRef.value?.validate(valid => {
    if (valid) {
      if (form.value.id != null) {
        updateRole(form.value as Role).then(res => {
...
// 控制删除
const handleDelete = (row: Partial<Role>) => {
...
// 控制更新
const handleUpdate = (row: Partial<Role>) => {
  resetFrom();
  const roleId = row.id || ids.value[0];
  queryRoleById(roleId).then(res => {
