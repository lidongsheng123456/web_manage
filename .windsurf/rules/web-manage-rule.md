...
## 6. 业务闭环规范 (Business Flow Completeness)

### 6.1 核心原则
开发业务模块时，**必须形成完整的业务闭环**，不能只做独立的 CRUD 表，表之间要有数据流转和状态变化。

### 6.2 业务闭环参考模式
| 业务类型 | 完整流程 | 关键表关联 |
| :--- | :--- | :--- |
| **电商/商城** | 浏览商品 → 加入购物车 → 下单 → 模拟支付 → 订单状态流转 | 商品表 → 购物车表 → 订单表 → 订单详情表 |
| **书店/图书馆** | 浏览图书 → 借书/买书 → 还书/确认收货 → 评价 | 图书表 → 借阅/订单表 → 归还记录表 |
| **外卖/餐饮** | 浏览店铺/菜品 → 加入购物车 → 下单 → 支付 → 配送状态 | 店铺表 → 菜品表 → 购物车 → 订单表 |
| **预约/服务** | 浏览服务 → 预约时间 → 支付 → 服务完成 → 评价 | 服务表 → 预约表 → 支付记录 → 评价表 |
| **旅游/景点** | 浏览景点/路线 → 预订 → 支付 → 出票 → 评价 | 景点表 → 路线表 → 订单表 → 门票表 |

### 6.3 订单状态流转规范
所有涉及交易的业务，订单必须有完整的状态机：
```
待支付(0) → 已支付/待发货(1) → 已发货/进行中(2) → 已完成(3) → 已取消(-1)
```
- 使用字典 `sys_dict_data` 管理订单状态
- 后台管理可查看/修改所有状态
- 前台用户只能触发有限的状态变更（如：支付、确认收货、取消）

### 6.4 模拟支付模块
项目内置模拟支付宝收银台页面，用于演示支付流程：
- **页面路径**: `views/front/home/SimulatePayView.vue`
- **路由**: `/SimulatePay`（独立全屏页面，不嵌入 FrontView 布局）
- **调用方式**: 前台下单后通过 `window.open` 跳转，传递订单参数
```js
// 跳转模拟支付
const payUrl = `/SimulatePay?payNo=${orderNo}&totalPrice=${price}&orderIds=${ids}&shopName=${name}`
window.open(payUrl, '_blank', 'width=1100,height=700')
```
- **支付回调**: 支付成功后调用后端接口更新订单状态，页面自动关闭

### 6.5 禁止事项
- **禁止只有孤立的 CRUD 表**，没有表间数据流转
- **禁止缺少状态字段**的业务表（订单、预约等必须有 status）
- **禁止前台只有列表展示**而没有用户交互操作（收藏/下单/评价）